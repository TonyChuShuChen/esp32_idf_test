...
static void dosegfault2(int i) {
	if (i>3) dosegfault3(i);
}

static void dosegfault(int i) {
	if (i<5) dosegfault(i+1);
	dosegfault2(i);
}
#endif
...
//	reg_val = READ_PERI_REG(UART_CONF1(0));
	...
	REG_SET_FIELD(PRO_UART_INTR_MAP_REG, DPORT_PRO_UART_INTR_MAP, ETS_UART_INUM);
	REG_SET_FIELD(PRO_UART1_INTR_MAP_REG, DPORT_PRO_UART1_INTR_MAP, ETS_UART_INUM);

	xt_set_interrupt_handler(ETS_UART_INUM, uartIsrHdl, NULL);
...
TEST_CASE("Bunch of FreeRTOS tests", "[freertos]")
{
	...
	for (i=0; i<12; i++) vTaskDelete(th[i]);
	xt_ints_off(1<<ETS_UART_INUM);
}

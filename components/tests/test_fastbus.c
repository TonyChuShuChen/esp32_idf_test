...
static void tskOne(void *pvParameters) {
	...
	while(1) {
		state=1;
		for (x=0; x<64; x++) WRITE_PERI_REG(UART_FIFO_REG(1), x^xor);
		for (w=0; w<(1<<14); w++); //delay
		state=2;
		test_fastbus_cp(UART_FIFO_REG(1), &res[0], 64, &dummy);
		for (w=0; w<(1<<10); w++); 			//delay
		for (x=0; x<255; x++) ct[x]=0;		//zero ctrs
		for (x=0; x<128; x++) ct[(int)res[x]^xor]++;	//count values
		...
		run++;
		if ((run&255)==0) printf("Loop %d errct %d\n", run, err);
		xor=(xor+1)&0xff;
	}
}
...
static void tskTwo(void *pvParameters) {
	...
	while(1) {
		while(state!=1) ;
		for (x=64; x<128; x++) WRITE_PERI_REG(UART_FIFO_REG(1), x^xor);
		while(state!=2);
...
//	CLEAR_PERI_REG_MASK(UART_INT_ENA_REG(1), UART_TXFIFO_EMPTY_INT_ENA|UART_RXFIFO_TOUT_INT_ENA);
	...
	for (i=0; i<2; i++) vTaskDelete(th[i]);

...
//	WRITE_PERI_REG(GPIO_FUNC11_OUT_SEL_CFG, (I2S0O_BCK_OUT_IDX<<GPIO_GPIO_FUNC0_OUT_SEL_S));
	...
	WRITE_PERI_REG(I2SCONF, 0);//I2S_I2S_SIG_LOOPBACK);
	WRITE_PERI_REG(I2SCONF2, 0);
	...
	WRITE_PERI_REG(I2SCONF1, I2S_RX_PCM_BYPASS|I2S_TX_PCM_BYPASS);

	WRITE_PERI_REG(I2SCONF_CHAN, (2<<I2S_TX_CHAN_MOD_S)|(2<<I2S_RX_CHAN_MOD_S));
...
//	for (i=0; i<(1<<8); i++);
	...
	CLEAR_PERI_REG_MASK(I2SOUT_LINK, I2S_I2S_OUTLINK_START|I2S_I2S_INLINK_START);

	  SET_PERI_REG_MASK(I2SCONF,I2S_I2S_TX_RESET|I2S_I2S_TX_FIFO_RESET|I2S_I2S_RX_RESET|I2S_I2S_RX_FIFO_RESET);
	CLEAR_PERI_REG_MASK(I2SCONF,I2S_I2S_TX_RESET|I2S_I2S_TX_FIFO_RESET|I2S_I2S_RX_RESET|I2S_I2S_RX_FIFO_RESET);
...
static void sendRecvBufDma(uint16_t *sbuf, uint16_t *rbuf, int len) {
	...
	//Reset I2S FIFO
	  SET_PERI_REG_MASK(I2SCONF,I2S_I2S_RX_RESET|I2S_I2S_TX_RESET|I2S_I2S_TX_FIFO_RESET|I2S_I2S_RX_FIFO_RESET);
	CLEAR_PERI_REG_MASK(I2SCONF,I2S_I2S_RX_RESET|I2S_I2S_TX_RESET|I2S_I2S_TX_FIFO_RESET|I2S_I2S_RX_FIFO_RESET);

	//Set desc addr
	CLEAR_PERI_REG_MASK(I2SOUT_LINK, I2S_I2S_OUTLINK_ADDR);
	SET_PERI_REG_MASK(I2SOUT_LINK, ((uint32_t)(&dmaDesc[0]))&I2S_I2S_OUTLINK_ADDR);
	CLEAR_PERI_REG_MASK(I2SIN_LINK, I2S_I2S_INLINK_ADDR);
	SET_PERI_REG_MASK(I2SIN_LINK, ((uint32_t)(&dmaDesc[1]))&I2S_I2S_INLINK_ADDR);

	SET_PERI_REG_MASK(I2S_FIFO_CONF,I2S_I2S_DSCR_EN); //Enable DMA mode

	WRITE_PERI_REG(I2SRXEOF_NUM, len);

	//Enable and configure DMA
	WRITE_PERI_REG(I2S_LC_CONF, I2S_I2S_OUT_DATA_BURST_EN  | 
			I2S_I2S_OUT_EOF_MODE | I2S_I2S_OUTDSCR_BURST_EN|I2S_I2S_OUT_DATA_BURST_EN |
			I2S_I2S_INDSCR_BURST_EN|I2S_I2S_MEM_TRANS_EN);

	//Start transmission
	SET_PERI_REG_MASK(I2SOUT_LINK, I2S_I2S_OUTLINK_START);
	SET_PERI_REG_MASK(I2SIN_LINK, I2S_I2S_INLINK_START);
...
//	lcdFlush();
	while(1) {
		for (x=0; x<DMALEN; x++) sbuf[x]=x^xorval;
		for (x=0; x<DMALEN; x++) rbuf[x]=0; //clear rbuf
		sendRecvBufDma(sbuf, rbuf, DMALEN);
		vTaskDelay(20/portTICK_PERIOD_MS);
		finishDma();
		for (x=0; x<DMALEN; x++) if (rbuf[x]!=(x^xorval)) printf("Rxbuf err! pos %d val %x xor %x", x, (int)rbuf[x], (int)xorval);
		printf(".");
		fflush(stdout);
		xorval++;
	}
...
static void tskOne(void *pvParameters) {
	...
	while(1) {
		ctr=0; lock=0; 
		state=1;
		for (x=0; x<16*1024; x++) test_s32c1i_lock(&lock, 1, 0, &ctr);
...
static void tskTwo(void *pvParameters) {
	...
	volatile int w;
	int delay;
	for (x=0; x<100; x++) *p++=*s++; //copy routine to different pool
...
TEST_CASE("S32C1I vs AHB test (needs I2S)", "[hw]")
{
	...
	for (i=0; i<3; i++) vTaskDelete(th[i]);
}
